...
// its timer fires, FURNACE_SURVIVORS decides what's left: ore smelts into
// an ingot, a survivor (currently just ingot) reappears unchanged, and
// anything else melts away with nothing left.
...
export function dumpInFurnace(
  ...
  const ms = item === 'ore' ? ORE_SMELT_MS : ITEM_MELT_MS;
  state.smelters.set(x + ',' + y, { x, y, item, readyAt: now + ms });
  return outcomeFor(item);
}

// per-tick: any job past its timer resolves and is removed — ore becomes
// an ingot, a survivor reappears as itself, anything else just vanishes.
export function updateSmelters(state: GameState, now: number): void {
  for (const [key, job] of state.smelters) {
    ...
    if (outcome !== 'destroyed') {
      const type = job.item === 'ore' ? 'ingot' : job.item;
