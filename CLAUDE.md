...
- **`systems/`** — game logic:
  ...
  - `pathfinding.ts`: generic 4-directional BFS (`findPath`, `bfsToAdjacent`) and line-of-sight, parameterized by a `walkable`/`isWall` callback so it has no game-state coupling.
